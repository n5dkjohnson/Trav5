...
import {Trav5AttackDialog } from "../helpers/attack-dialog.mjs";
 ...
 * @extends {Item}
 */
export class Trav5Item extends Item {
        ...
        if (this.actor) {
            ...
            if (this.actor.type === "spacecraft") {
                if (this.type === "term" || this.type === "associate") {
                    ui.notifications.warn(game.i18n.format("TRAV5.Warn.Drop.OnlyOnCharacter",
                ...
                if (this.type === "cargo" || this.type === "hardware" || this.type === "role") {
                    ui.notifications.warn(game.i18n.format("TRAV5.Warn.Drop.NotOnCharacter",
                        { item: this.name, type: this.type }));
                    return false;
                }
                if (this.actor.type === "npc" || this.actor.type === "creature") {
                    if (this.type === "term" || this.type === "associate") {
                        ui.notifications.warn(game.i18n.format("TRAV5.Warn.Drop.CareerOnNPC"))
                        return false;
                    }
                }
            }
    ...
    async roll() {
        ...
        if (item.type === "weapon") {
            if (!quickRoll) {
                new Trav5AttackDialog(this.actor, item).render(true);
            } else {
    ...
    printWeaponTraits() {
        ...
        if (this.type === "weapon" && this.system.weapon) {
            ...
            for (let t of traits) {
                ...
                if (CONFIG.TRAV5.WEAPONS.traits[trait]) {
                    let label = game.i18n.localize("TRAV5.Item.WeaponTrait.Label." + trait);
                    if (text) {
                        text += ", ";
                    }
                    text += label;
    ...
    statusClick() {
        if (this.type === "hardware") {
            if (this.system.status === Trav5Item.ACTIVE) {
                this.system.status = Trav5Item.INACTIVE;
            } else if (this.system.status === Trav5Item.INACTIVE) {
                this.system.status = Trav5Item.ACTIVE;
            }
            this.update({"system.status": this.system.status});
        } else if (this.type === "software") {
            if (this.system.status === Trav5Item.RUNNING) {
                this.system.status = Trav5Item.INACTIVE;
            } else if (this.system.status === Trav5Item.INACTIVE) {
                this.system.status = Trav5Item.RUNNING;
            }
            this.update({"system.status": this.system.status});
        }
    }
