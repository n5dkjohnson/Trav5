...
import {Trav5SpacecraftDamageDialog} from "../helpers/spacecraft-damage-dialog.mjs";

/**
 * Extend the base Actor document by defining a custom roll data structure which is ideal for the Simple system.
 * @extends {Actor}
 */
export class Trav5Actor extends Actor {
    ...
    prepareBaseData() {
        ...
        for (const effect of this.effects) {
            ...
            if (effect.origin) {
                ...
                if (item) {
                    effect.isSuppressed = item.system.status !== Trav5Item.EQUIPPED;
                }
            }
        }
    }
    ...
    _preUpdate(changes, options, user) {
        if (this.type === "spacecraft") {
            if (changes?.system?.spacecraft?.computer) {
                ...
                if (core) {
                    changes.system.spacecraft.computer.processing = CONFIG.TRAV5.COMPUTERS.techLevel[tl].core;
                } else {
                    changes.system.spacecraft.computer.processing = CONFIG.TRAV5.COMPUTERS.techLevel[tl].computer;
                }
            }
        }
    }
...
*/
    ...
    _prepareSpacecraftData(actor) {
        ...
        if (this.getFlag("trav5","initPilotDM")) {
            init.value += parseInt(this.getFlag("trav5", "initPilotDM"));
        }
        if (this.getFlag("trav5","initTacticsDM")) {
            init.value += parseInt(this.getFlag("trav5", "initTacticsDM"));
        }
    }
  ...
  applyDamageToPerson(damage, options) {
      ...
      for (let i of this.items) {
          if (i.type === "weapon" && i.system.status === Trav5Item.EQUIPPED) {
      ...
      ui.notifications.info(game.i18n.format("TRAV5.Info.Damage",
      ...
      if (this.type === "traveller") {
          ...
          if (options.noUI) {
              ...
              new Trav5DamageDialog(this, damage, options).render(true);
          }
      } else {
  ...
  applyActualDamageToPerson(damage, options) {
      ...
      if (stun) {
          ...
          if (stuns > 0) {
              this.setFlag("trav5", "stunned", true);
              this.setFlag("trav5", "stunnedRounds",
                  this.getFlag("trav5", "stunnedRounds") ?
                      parseInt(this.getFlag("trav5", "stunnedRounds")) + stuns : stuns
              );
          }
      } else {
          hits.damage += damage;
  ...
  applyDamageToSpacecraft(damage, options) {
      ...
      new Trav5SpacecraftDamageDialog(this, damage, options).render(true);
  }
  ...
  getSkillIcon(skillId) {
      ...
      if (!skill) {
          return "systems/trav5/icons/skills/untrained.svg";
      }
      if (skill.icon) {
          return skill.icon;
      }
      return `systems/trav5/icons/skills/${skillId}.svg`;
  }
  ...
  getSkillLabel(skill, showValue) {
      ...
      if (skill.label) {
          ...
          text = game.i18n.localize("TRAV5.Skills." + skillId);
          if (text.indexOf("TRAV5.Skills") === 0) {
              text = skillId;
          }
      }
      if (specId) {
          if (skill.specialities && skill.specialities[specId]) {
              ...
              if (spec.label) {
                ...
                let label =  game.i18n.localize("TRAV5.Skills." + specId);
                if (label.indexOf("TRAV5.Skills") === 0) {
                    text += ` (${specId})`;
                } else {
                    text += ` (${label})`;
                }
  ...
  getCriticalLevel(critical) {
      if (this.type === "spacecraft") {
          if (TRAV5.SPACECRAFT_CRITICALS[critical]) {
              let value = this.getFlag("trav5", "crit_" + critical);
  ...
  setCriticalLevel(critical, level) {
      if (this.type === "spacecraft") {
          ...
          if (level < 1) {
              this.unsetFlag("trav5", "crit_" + critical);
              let hasCrits = false;
              for (let c in TRAV5.SPACECRAFT_CRITICALS) {
                  if (c != critical && this.flags.trav5["crit_"+c]) {
                      hasCrits = true;
                      break;
                  }
              }
              this.setFlag("trav5", "hasCrits", hasCrits);
          } else if (TRAV5.SPACECRAFT_CRITICALS[critical]) {
              this.setFlag("trav5", "crit_" + critical, level);
              this.setFlag("trav5", "hasCrits", true);
