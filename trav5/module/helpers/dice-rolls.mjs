...
export function skillLabel(skill, skillId) {
    ...
    if (skill.label && skill.label.length > 0) {
        ...
        let label = game.i18n.localize("TRAV5.Skills." + skillId);
...
export async function rollAttack(actor, weapon, skillDM, dm, rollType, range, autoOption, isParry) {
    ...
    if (actor && actor.flags?.trav5?.reaction) {
        let react = Math.abs(parseInt(actor.flags.trav5.reaction));
        if (react !== 0) {
            dice += ` - ${react}[Dodge]`;
        }
    ...
    for (let attack=1; attack <= attacks; attack++) {
        ...
        if (effect <= -5 && (hasTrait(weapon.system.weapon.traits, "dangerous")||hasTrait(weapon.system.traits, "veryDangerous"))) {
            effectText = game.i18n.localize("TRAV5.Attack.Dangerous");
...
export async function rollSpaceAttack(starship, gunner, weaponItem, options) {
    ...
    let multiplier = 1;
    if (CONFIG.TRAV5.SPACE_MOUNTS[mount]) {
        multiplier = parseInt(CONFIG.TRAV5.SPACE_MOUNTS[mount].multiplier);
    }
        ...
        <div class="attack-message">
            ...
            <div class="message-content">
                <div>
                    ...
                    ${game.i18n.localize("TRAV5.Item.SpaceRange." + options.range)}<br/>
...
function getDifficultyLabel(difficulty) {
    if (difficulty >= 0 && difficulty <= 16) {
        return game.i18n.localize("TRAV5.TaskDifficulty." + difficulty);
    }
    return "";
}
...
export async function rollSkill(actor, skill, speciality, cha, dm, rollType, difficulty) {
    ...
    if (cha) {
        ...
        let reaction = actor.getFlag("trav5", "reaction");
        if (reaction) {
    ...
    if (game.settings.get("trav5", "verboseSkillRolls")) {
    ...
    if (roll) {
        ...
        if (game.settings.get("trav5", "useChatIcons")) {
