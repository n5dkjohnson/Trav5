...
import {Trav5DamageDialog} from "../damage-dialog.mjs";
...
Tools.setStatusFor = function(actor, args, status) {
    if (args.includes(status)) {
        actor.setFlag("trav5", status, true);
    }
    if (args.includes("-" + status)) {
    ...
    if (actor.type === "traveller") {
        ...
        if (stun && data.characteristics) {
            if (data.hits.tmpDamage > data.characteristics.END.value) {
                ...
                actor.setFlag("trav5", "stunned", true);
                actor.setFlag("trav5", "stunnedRounds",
                    actor.getFlag("trav5", "stunnedRounds")?
                        parseInt(actor.getFlag("trav5", "stunnedRounds"))+remaining:remaining);
            }
        }
    }
...
Tools.applyDamageToTokens = function(damage, damageOptions) {
    ...
    if (tokens.size === 0) {
        ui.notifications.error(game.i18n.localize("TRAV5.Error.CombatNoSelection"));
        return;
    }
...
Tools.currentTime = function(chatData, args) {
    const user = game.users.current;
    let year = game.settings.get("trav5", "currentYear");
    let day = game.settings.get("trav5", "currentDay");
    ...
    if (user.isGM && args.length > 0) {
        ...
        game.settings.set("trav5", "currentYear", year);
        game.settings.set("trav5", "currentDay", day);
