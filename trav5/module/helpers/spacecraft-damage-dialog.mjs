...
export class Trav5SpacecraftDamageDialog extends Application {
    static get defaultOptions() {
        const options = super.defaultOptions;
        options.template = "systems/trav5/templates/spacecraft-damage-dialog.html";
        options.width = "auto";
        options.height = "auto";
        options.title = game.i18n.localize("TRAV5.SpacecraftDamageDialog.Title");
...
//    constructor(actor, damage, ap, laser, traits) {
    constructor(actor, damage, damageOptions) {
        ...
        for (let c in TRAV5.SPACECRAFT_CRITICALS) {
            let severity = this.actor.flags.trav5["crit_"+c];
            if (severity) {
        ...
        let roll = 2;
        for (let c in TRAV5.SPACECRAFT_CRITICALS) {
            this.criticalLabels[c] = game.i18n.format("TRAV5.Spacecraft.Criticals." + c) + ` (${roll})`;
            roll++;
        }
    }
