...
import {Trav5SkillDialog } from "../helpers/skill-dialog.mjs";
import {Trav5XPDialog } from "../helpers/xp-dialog.mjs";
import {Trav5QuantityDialog } from "../helpers/quantity-dialog.mjs";
import {Trav5DamageDialog } from "../helpers/damage-dialog.mjs";
import {Trav5AddSkillDialog } from "../helpers/add-skill-dialog.mjs";
import {Trav5CrewMemberDialog } from "../helpers/crew-member-dialog.mjs";
import {Trav5SpacecraftAttackDialog } from "../helpers/spacecraft-attack-dialog.mjs";
 ...
 * @extends {ActorSheet}
 */
export class Trav5ActorSheet extends ActorSheet {
    static BEHAVIOUR = [
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            classes: ["trav5", "sheet", "actor"],
            template: "systems/trav5/templates/actor/actor-sheet.html",
            width: 720,
            height: 600,
    ...
    /** @override */
    get template() {
        return `systems/trav5/templates/actor/actor-${this.actor.type}-sheet.html`;
    }
    ...
    async getData() {
        ...
        context.currentYear = game.settings.get("trav5", "currentYear");
        ...
        if (type === 'traveller' || type === 'package') {
            ...
            if (actorData.settings.autoAge) {
                actorData.sophont.age = parseInt(game.settings.get("trav5", "currentYear")) - actorData.birthYear;
            }
        } else if (type === 'npc') {
            this._prepareItems(context);
        ...
        if (type === "creature") {
            ...
            for (let b in CONFIG.TRAV5.CREATURES.behaviours) {
                if (actorData.behaviour.indexOf(b) === -1) {
                    context.behaviours[b] = game.i18n.localize("TRAV5.Creature.Behaviour." + b);
                } else {
                    context.haveBehaviours[b] = {
                        "label": game.i18n.localize("TRAV5.Creature.Behaviour." + b),
                        "title": game.i18n.localize("TRAV5.Creature.BehaviourText." + b)
                    }
                }
            }
            context.traits = {};
            context.traits[""] = "";
            for (let t in CONFIG.TRAV5.CREATURES.traits) {
                if (actorData.traits.indexOf(t) === -1) {
                    if (!CONFIG.TRAV5.CREATURES.traits[t].conflict ||
                        actorData.traits.indexOf(CONFIG.TRAV5.CREATURES.traits[t].conflict) === -1) {
                        context.traits[t] = game.i18n.localize("TRAV5.Creature.Trait." + t);
                    }
                }
            }
            if (CONFIG.TRAV5.CREATURES.sizes[actorData.size]) {
                context.suggestedHits = game.i18n.format("TRAV5.TravellerSheet.SizeRecommendedHits",
                    {
                        "min": CONFIG.TRAV5.CREATURES.sizes[actorData.size].minHits,
                        "max": CONFIG.TRAV5.CREATURES.sizes[actorData.size].maxHits
                    });
            } else {
            ...
            for (let tl = 7; tl <= 21; tl++) {
                context.selectShipTL[`${tl}`] = `${tl} - ${game.i18n.localize("TRAV5.Item.Tech." + tl)}`;
            }
            context.selectShipConfig = {};
            for (let c in CONFIG.TRAV5.SHIP_CONFIGURATION) {
                context.selectShipConfig[c] = game.i18n.localize("TRAV5.Spacecraft.Configuration." + c);
            }
            ...
            context.selectSystemTypes = {
                "": "",
                "armour": game.i18n.localize("TRAV5.Spacecraft.System.armour"),
                "bridge": game.i18n.localize("TRAV5.Spacecraft.System.bridge"),
                "cargo": game.i18n.localize("TRAV5.Spacecraft.System.cargo"),
                "computer": game.i18n.localize("TRAV5.Spacecraft.System.computer"),
                "dock": game.i18n.localize("TRAV5.Spacecraft.System.dock"),
                "fuel": game.i18n.localize("TRAV5.Spacecraft.System.fuel"),
                "j-drive": game.i18n.localize("TRAV5.Spacecraft.System.j-drive"),
                "m-drive": game.i18n.localize("TRAV5.Spacecraft.System.m-drive"),
                "r-drive": game.i18n.localize("TRAV5.Spacecraft.System.r-drive"),
                "power": game.i18n.localize("TRAV5.Spacecraft.System.power"),
                "stateroom": game.i18n.localize("TRAV5.Spacecraft.System.stateroom"),
                "weapon": game.i18n.localize("TRAV5.Spacecraft.System.weapon")
            };
        } else if (type === "vehicle") {
            context.selectVehicleTL = {};
            for (let tl = 0; tl <= 17; tl++) {
                context.selectVehicleTL[`${tl}`] = `${tl} - ${game.i18n.localize("TRAV5.Item.Tech." + tl)}`;
            }
            context.selectChassis = {};
            context.selectSubType = {};
            for (let c in CONFIG.TRAV5.VEHICLES.CHASSIS) {
                if (!actorData.vehicle.chassis) {
                    actorData.vehicle.chassis = c;
                }
                context.selectChassis[c] = game.i18n.localize("TRAV5.Vehicle.ChassisLabel." + c);
                if (actorData.vehicle.chassis === c) {
                    for (let s in CONFIG.TRAV5.VEHICLES.CHASSIS[c].subtypes) {
                        context.selectSubType[s] = game.i18n.localize("TRAV5.Vehicle.SubTypeLabel." + s);
                    }
                }
            }
            context.selectSpeed = {};
            let lastSpeed = 0;
            for (let spd in CONFIG.TRAV5.VEHICLES.SPEED) {
                if (!actorData.vehicle.speed) {
                    actorData.vehicle.speed = spd;
                }
                let label = game.i18n.localize("TRAV5.Vehicle.SpeedBand." + spd);
                let max = CONFIG.TRAV5.VEHICLES.SPEED[spd].max;
                if (lastSpeed || max) {
    ...
    _prepareSpacecraftItems(context) {
        ...
        if (actorData.spacecraft.configuration) {
            config = TRAV5.SHIP_CONFIGURATION[actorData.spacecraft.configuration];
            if (config) {
        ...
        for (let i of context.items) {
            if (i.type === 'cargo') {
                ...
                software.push(i);
                if (parseInt(i.system.software.bandwidth) > 0 && i.system.status === Trav5Item.RUNNING) {
                ...
                if (i.system.status === Trav5Item.ACTIVE) {
                    if (h.system === "power") {
                ...
                if (h.system === "j-drive" && i.system.status === Trav5Item.ACTIVE) {
                    jdrive = Math.max(jdrive, parseInt(h.rating));
                }
                if (h.system === "m-drive" && i.system.status === Trav5Item.ACTIVE) {
                    mdrive = Math.max(mdrive, parseInt(h.rating));
                }
                if (h.system === "r-drive" && i.system.status === Trav5Item.ACTIVE) {
    ...
    _prepareItems(context) {
        ...
        for (let i of context.items) {
            ...
            if (i.system.weight !== undefined) {
                if (i.system.status === Trav5Item.CARRIED) {
                    weight += parseFloat(i.system.weight) * parseFloat(i.system.quantity);
                } else if (i.system.status === Trav5Item.EQUIPPED) {
                    if (i.type === "armour") {
            ...
            if (i.type === 'weapon') {
                weapons.push(i);
                if (i.system.status === Trav5Item.EQUIPPED) {
        ...
        let wasEncumbered = !!this.actor.getFlag("trav5", "encumbered");
        let wasVaccSuit = !!this.actor.getFlag("trav5", "vaccSuit");
        let isVaccSuit = false;

        if ( game.settings.get("trav5", "useEncumbrance")) {
            if (weight > this.actor.system.heavyLoad) {
                this.actor.system.modifiers.encumbrance.auto = -2;
                if (!wasEncumbered) {
                    this.actor.setFlag("trav5", "encumbered", true);
                }
            } else if (wasEncumbered) {
                this.actor.setFlag("trav5", "encumbered", false);
            }
        }
        ...
        if (isVaccSuit !== wasVaccSuit) {
            if (isVaccSuit) {
                ...
                //this.actor.setFlag("trav5", "vaccSuit", true);
            } else {
                // Causes infinite loop. Why?
                //this.actor.setFlag("trav5", "vaccSuit", false);
            }
        }
    ...
    _setItemStatus(actor, item, status) {
        ...
        if (item.type === "armour") {
            ...
            if (status === Trav5Item.EQUIPPED) {
                for (let i of actor.items) {
                    if (i.system.armour && i.system.status === Trav5Item.EQUIPPED && i.system.armour.form === form) {
                        i.system.status = Trav5Item.CARRIED;
                        i.update({"system.status": Trav5Item.CARRIED });
                    }
                }
            }
        }
        const isActive = (status === Trav5Item.EQUIPPED);
        ...
        if (context.actor && (context.actor.type === 'traveller' || context.actor.type === 'npc' || context.actor.type === 'creature')) {
            ...
            for (let i of context.items) {
                if (i.system.armour) {
                    ...
                    if (armourData.form === "natural" || i.system.status === Trav5Item.EQUIPPED) {
  ...
  /* -------------------------------------------- */
    ...
    activateListeners(html) {
                ...
                for (let c in parent.system.crewed.crew) {
                    if (crew[c][item._id]) {
                        ...
                        ui.notifications.error(
                            game.i18n.format("TRAV5.Error.RoleInUse",
                ...
                if (parent && parent.type === "spacecraft" && item.system.weapon.scale === "spacecraft") {
                    ...
                    for (let i of parent.items) {
                        if (i.type === "hardware" && i.system.hardware.system === "weapon") {
                            ...
                            if (wpns[item._id]) {
                                ui.notifications.error(
                                    game.i18n.format("TRAV5.Error.WeaponInUse",
        ...
        html.find('.item-activate').click(ev => {
            const li = $(ev.currentTarget).parents(".item");
            const item = this.actor.items.get(li.data("itemId"));
            this._setItemStatus(this.actor, item, Trav5Item.EQUIPPED);
        });
        html.find('.item-deactivate').click(ev => {
            const li = $(ev.currentTarget).parents(".item");
            const item = this.actor.items.get(li.data("itemId"));
            this._setItemStatus(this.actor, item, Trav5Item.CARRIED);
        });
        html.find('.item-store').click(ev => {
            const li = $(ev.currentTarget).parents(".item");
            const item = this.actor.items.get(li.data("itemId"));
            this._setItemStatus(this.actor, item, Trav5Item.OWNED);
        });
        ...
        html.find('.crew-edit').click(ev => {
            ...
            new Trav5CrewMemberDialog(crew, this.actor, this).render(true);
    ...
    async _creatureSelectBehaviour(selectedBehaviour) {
        ...
        let b = TRAV5.CREATURES.behaviours[selectedBehaviour].skills;
        if (!b) return;
    ...
    async _creatureRemoveBehaviour(removedBehaviour) {
        if (this.actor.system.behaviour) {
            ...
            let b = TRAV5.CREATURES.behaviours[removedBehaviour].skills;
            if (!b) return;
            let skills = this.actor.system.skills;
            for (let s in b) {
                let skill = b[s];
    ...
    async _creatureSelectTrait(selectedTrait) {
        const traitData = TRAV5.CREATURES.traits[selectedTrait];
        if (traitData) {
            ...
            if (traitData.set) {
                ...
                for (let s in traitData.skills) {
                    ...
                    if (skills[skill.skill]) {
                        ...
                        skills[skill.skill].notes = game.i18n.localize("TRAV5.Creature.Trait."+selectedTrait);
    ...
    async _creatureRemoveTrait(trait) {
        const traitData = TRAV5.CREATURES.traits[trait];
        if (traitData) {
    ...
    _rollDodge(event, actor) {
        ...
        if (dodge > 0) {
            let current = actor.getFlag("trav5", "reaction");
            if (!current) current = 0;

            actor.setFlag("trav5", "reaction", parseInt(current) - 1);
        }
    }

    _clearDodge(actor) {
        actor.unsetFlag("trav5", "reaction");
    }
    ...
    async _clearStunned(actor) {
        ...
        await actor.unsetFlag("trav5", "stunned");
        await actor.unsetFlag("trav5", "stunnedRounds");
    }

    _clearStatus(actor, status) {
        actor.unsetFlag("trav5", status);
    }
    ...
    async _runCrewAction(shipActor, actorCrewId, roleId, actionId) {
        ...
        if (!actorCrew) {
            ui.notifications.warn(game.i18n.format("TRAV5.Warn.Crew.NoCrewActor", { crewId: actorCrewId}));
            return;
        }
        const itemRole = shipActor.items.get(roleId);
        if (!itemRole) {
            ui.notifications.warn(game.i18n.format("TRAV5.Warn.Crew.NoCrewActor", { roleId: roleId}));
            return;
        }
        ...
        if (action.action === "chat") {
            ...
            let chatData = {
                ...
                speaker: {
                    actor: actorCrew._id,
                    alias: game.i18n.format("TRAV5.Role.ChatAlias", {
            ...
            new Trav5SkillDialog(actorCrew, skill, spec, cha, parseInt(action.dm?action.dm:0), target).render(true);
            ...
            new Trav5SpacecraftAttackDialog(shipActor, actorCrew, weaponItem, dm).render(true);
        } else if (action.action === "special") {
            if (action.special === "pilot") {
                let pilotDM = actorCrew.getSkillValue("pilot.spacecraft");
                shipActor.setFlag("trav5", "initPilotDM", pilotDM);
                shipActor.setFlag("trav5", "initPilotName", actorCrew.name);
                ...
                shipActor.setFlag("trav5", "initTacticsDM", roll.total);
                shipActor.setFlag("trav5", "initTacticsName", actorCrew.name);
    ...
    async _onDropActor(event, data) {
        ...
        if ((this.actor.type === "spacecraft" || this.actor.type === "vehicle") && (actor.type === "traveller" || actor.type === "npc")) {
            ...
            ui.notifications.info(game.i18n.format("TRAV5.Info.Drop.DropPackage",
            ...
            for (let item of actor.items) {
                ...
                ui.notifications.info(game.i18n.format("TRAV5.Info.Drop.DropPackageItem",
                    { item: item.name, actor: this.actor.name }));
                await Item.create(itemData, {parent: this.actor});

                if (itemData.type !== "term") {
                    if (itemData.type === "associate") {
                        let rel = game.i18n.localize("TRAV5.History.Relation." + item.system.associate.relationship);
            ...
            if (game.users.current.isGM) {
                if (game.settings.get("trav5", "gmSheetNotification") === "private") {
                    who = [game.user.id];
                }
            } else {
                if (game.settings.get("trav5", "playerSheetNotification") === "private") {
                    who = [game.user.id];
                } else if (game.settings.get("trav5", "playerSheetNotification") === "gm") {
        ...
        if (actor.uuid.indexOf(srcActorId) === -1) {
            ...
            if (srcActor) {
                ...
                if (parseInt(item.system.quantity) > 1) {
                    new Trav5QuantityDialog(srcActor, actor, item).render(true);
                } else {
    ...
    _createHardware(systemType) {
        ...
        if (systemType === "j-drive") {
            itemName = "J-Drive";
            img = "systems/trav5/icons/hardware/j-drive.svg";
            system.tl = 9;
            ...
            itemName = "M-Drive";
            img = "systems/trav5/icons/hardware/m-drive.svg";
            system.tl = 9;
            system.hardware.rating = 1;
            ...
            itemName = "R-Drive";
            img = "systems/trav5/icons/hardware/r-drive.svg";
            system.tl = 7;
            system.hardware.rating = 1;
            ...
            img = "systems/trav5/icons/hardware/fusion_reactor.svg";
            system.tl = 8;
            ...
            img = "systems/trav5/icons/hardware/armour_crystaliron.svg";
            system.tl = 10;
            ...
            itemName = "Fuel Tanks";
            img = "systems/trav5/icons/hardware/fuel_tank.svg";
            system.tl = 9;
            system.hardware.rating = 10;
        } else if (systemType === "cargo") {
            itemName = "Cargo Hold";
            img = "systems/trav5/icons/hardware/cargo_hold.svg";
            system.tl = 9;
            system.hardware.rating = 10;
        } else if (systemType === "weapon") {
            itemName = "Turret";
            img = "systems/trav5/icons/hardware/turret.svg";
            system.tl = 9;
            system.hardware.tonnage.tons = 1;
        } else if (systemType === "computer") {
            itemName = "Computer";
            img = "systems/trav5/icons/hardware/computer.svg";
            system.tl = this.actor.system.spacecraft.tl;
        } else if (systemType === "bridge") {
            itemName = "Bridge";
            img = "systems/trav5/icons/hardware/bridge.svg";
            ...
            itemName = "Stateroom";
            img = "systems/trav5/icons/hardware/stateroom.svg";
            system.hardware.system = "general";
    ...
    _onAddNewSkill(event, actor) {
        ...
        new Trav5AddSkillDialog(actor).render(true);
    }
...
//            if (data.skills[skill].trained) {
  ...
  //          }
        ...
        let quickRoll = game.settings.get("trav5", "quickRolls");
        if (event.shiftKey) {
            quickRoll = !quickRoll;
        }
        if (event.ctrlKey) {
            new Trav5XPDialog(actor, skill, spec, cha).render(true);
        } else if (!quickRoll) {
            new Trav5SkillDialog(actor, skill, spec, cha).render(true);
        } else {
...
/*
    async _onSubmit(event, updateData, preventClose, preventRender) {
        ...
        if (this.actor.type === "spacecraft" && game.settings.get("trav5", "autoResizeSpacecraft")) {
            let size = 1;
...
export class Trav5NPCActorSheet extends Trav5ActorSheet {
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            classes: ["trav5", "sheet", "actor"],
            template: "systems/trav5/templates/actor/actor-sheet.html",
...
export class Trav5CreatureActorSheet extends Trav5ActorSheet {
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            classes: ["trav5", "sheet", "actor"],
            template: "systems/trav5/templates/actor/actor-sheet.html",
