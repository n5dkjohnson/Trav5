 ...
 * @extends {ItemSheet}
 */
export class Trav5ItemSheet extends ItemSheet {

    /** @override */
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            classes: ["trav5", "sheet", "item"],
            width: 680,
            height: 480,
    ...
    /** @override */
    get template() {
        const path = "systems/trav5/templates/item";
    ...
    async getData() {
        ...
        context.effectTypes = CONFIG.TRAV5.EFFECTS;
        ...
        context.techLevels = {};
        for (let i=0; i <= 25; i++) {
            context.techLevels[i] = i + " (" + game.i18n.localize(`TRAV5.Item.Tech.${i}`) + ")";
        }

        context.characteristics = TRAV5.CHARACTERISTICS;
        ...
        if (context.item.type === "hardware") {
            ...
            context.selectSystemTypes = {
                "general": game.i18n.localize("TRAV5.Spacecraft.System.general"),
                ...
                "sensor": game.i18n.localize("TRAV5.Spacecraft.System.sensor"),
            };
        ...
        if (context.item.type === "hardware" && context.item.parent != null) {
            ...
            if (TRAV5.SPACECRAFT_ADVANTAGES[context.item.system.hardware.system]) {
                // List of prototype/advanced options.
                context.ADVANCES = TRAV5.SPACECRAFT_ADVANCES;
                context.ADVANCES_LIST = {};
                for (let a in context.ADVANCES) {
                    context.ADVANCES_LIST[a] = game.i18n.format("TRAV5.Spacecraft.Advances." + a);
                }
                ...
                context.ADVANTAGES = TRAV5.SPACECRAFT_ADVANTAGES[context.item.system.hardware.system];
                ...
                if (pointsAvailable !== 0) {
                    for (let a in context.ADVANTAGES) {
                        ...
                        context.ADVANTAGES_LIST[a] =
                            `${game.i18n.format("TRAV5.Spacecraft.Advantages." + a)} (${context.ADVANTAGES[a].cost})`;
                    }
                }
            }
            ...
            context.energyTypes[""] = "";
            context.haveEnergy = {};
            for (let e of CONFIG.TRAV5.WEAPONS.energyTypes) {
                if (context.item.system.armour.otherTypes.toLowerCase().indexOf(e) === -1) {
                    context.energyTypes[e] = game.i18n.localize("TRAV5.Item.EnergyType." + e);
                } else {
                    context.haveEnergy[e] = game.i18n.localize("TRAV5.Item.EnergyType." + e);
                }
            }
            ...
            context.weaponScale = {
                "traveller": game.i18n.localize("TRAV5.Item.Scale.traveller"),
                "vehicle": game.i18n.localize("TRAV5.Item.Scale.vehicle"),
                "spacecraft": game.i18n.localize("TRAV5.Item.Scale.spacecraft"),
            }
            context.weaponSpaceMount = {
                "fixed": game.i18n.localize("TRAV5.Item.SpaceMount.Fixed"),
                "turret": game.i18n.localize("TRAV5.Item.SpaceMount.Turret"),
                "barbette": game.i18n.localize("TRAV5.Item.SpaceMount.Barbette"),
                "bay.small": game.i18n.localize("TRAV5.Item.SpaceMount.BaySmall"),
                "bay.medium": game.i18n.localize("TRAV5.Item.SpaceMount.BayMedium"),
                "bay.large": game.i18n.localize("TRAV5.Item.SpaceMount.BayLarge"),
                "spinal": game.i18n.localize("TRAV5.Item.SpaceMount.Spinal"),
            }
            context.weaponSpaceRange = {
                "adjacent": game.i18n.localize("TRAV5.Item.SpaceRange.adjacent"),
                "close": game.i18n.localize("TRAV5.Item.SpaceRange.close"),
                "short": game.i18n.localize("TRAV5.Item.SpaceRange.short"),
                "medium": game.i18n.localize("TRAV5.Item.SpaceRange.medium"),
                "long": game.i18n.localize("TRAV5.Item.SpaceRange.long"),
                "verylong": game.i18n.localize("TRAV5.Item.SpaceRange.verylong"),
                "distant": game.i18n.localize("TRAV5.Item.SpaceRange.distant"),
            }
            context.energyTypes = {};
            context.energyTypes["standard"] = game.i18n.localize("TRAV5.Item.EnergyType.standard");
            for (let e of CONFIG.TRAV5.WEAPONS.energyTypes) {
                context.energyTypes[e] = game.i18n.localize("TRAV5.Item.EnergyType." + e);
            }
            ...
            for (let trait in CONFIG.TRAV5.WEAPONS.traits) {
                if (!hasTrait(traits, trait)) {
                    let t = CONFIG.TRAV5.WEAPONS.traits[trait];
                    if (t.scale) {
                    ...
                    context.weaponTraits[trait] = game.i18n.localize("TRAV5.Item.WeaponTrait.Label."+trait);
                }
            }
            let allSkills = TRAV5.SKILLS;
            if (context.item.parent && context.item.parent.system.skills) {
            ...
            for (let skillId in allSkills) {
                ...
                if (skill.specialities) {
                    for (let specId in skill.specialities) {
                        ...
                        if (spec.combat) {
                            let label = skill.label?skill.label:game.i18n.localize("TRAV5.Skills."+skillId);
                            let specLabel = spec.label?spec.label:game.i18n.localize("TRAV5.Skills."+specId);
                            context.combatSkills[skillId+"."+specId] = label + " (" + specLabel + ")";
                        }
                    }
                } else if (skill.combat) {
                    context.combatSkills[skillId] = skill.label?skill.label:game.i18n.localize("TRAV5.Skills."+skillId);
                }
            }
            ...
            if (!item.hasCargoAvailability("All")) {
                if (!item.system.cargo.availability) {
                    context.availability["All"] = game.i18n.localize("TRAV5.Trade.All");
                }
                for (let trait in CONFIG.TRAV5.TRADE.codes) {
                    if (!hasTrait(context.item.system.cargo.availability, trait)) {
                        context.availability[trait] = game.i18n.localize("TRAV5.Trade." + trait);
                    } else {
                        context.haveAvailability[trait] = game.i18n.localize("TRAV5.Trade." + trait);
                    }
                }
            } else {
                context.haveAvailability["All"] = game.i18n.localize("TRAV5.Trade.All");
                context.availability = null;
            }
            context.purchaseTraits[""] = "";
            for (let trait in CONFIG.TRAV5.TRADE.codes) {
                if (!hasTrait(item.system.cargo.purchaseDM, trait)) {
                    context.purchaseTraits[trait] = game.i18n.localize("TRAV5.Trade." + trait);
                }
            }
            context.saleTraits[""] = "";
            for (let trait in CONFIG.TRAV5.TRADE.codes) {
                if (!hasTrait(item.system.cargo.saleDM, trait)) {
                    context.saleTraits[trait] = game.i18n.localize("TRAV5.Trade." + trait);
                }
            }
    ...
    calculateHardware(context, item) {
        ...
        if (item.system.hardware.system === "computer") {
            ...
            if (tl > 6) {
                tl = "" + Math.min(15, tl);
                bw = TRAV5.COMPUTERS.techLevel[tl].computer;
                if (item.system.hardware.isComputerCore) {
                    bw = TRAV5.COMPUTERS.techLevel[tl].core;
                }
            }
        ...
        if (!item.system.status) {
            switch (item.system.hardware.system) {
                ...
                case "r-drive":case "computer": case "weapon":
                    item.system.status = Trav5Item.ACTIVE;
    ...
    async _modifyCargoTrait(trait, field, modifier) {
        ...
        const traitData = TRAV5.TRADE.codes[trait];
        if (traitData) {
    ...
    async _selectWeaponTrait(selectedTrait) {
        const traitData = TRAV5.WEAPONS.traits[selectedTrait];
        if (traitData) {
    ...
    async _modifyWeaponTrait(trait, modifier) {
        ...
        const traitData = TRAV5.WEAPONS.traits[trait];
        if (traitData) {
